...
import { messageRead } from "../../../redux/actions/chat.actions"

const DirectMessagesForm = ({ chat, authId, isChatCurrent, setNewChat, online, hasUnreadMsg }) => {
  ...
  return (
    ...
    >
     ...
     ></span>
      <span className="flex flex-1 items-start text-purple-100">{userToDm.login}</span>
      { hasUnreadMsg &&
              <svg
              className="h-6 w-6 mt-1 text-white stroke-current stroke-1 fill-current animate-pulse"
              viewBox="0 0 32 32"
            >
              <g id="surface1">
